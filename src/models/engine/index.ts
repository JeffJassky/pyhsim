...
import {
  ...
  getAllEnzymeKeys,
  ENZYMES,
} from "../physiology/pharmacology";

export const SIGNAL_DEFINITIONS: Partial<
 ...
 */
export function getAllUnifiedDefinitions(): Record<
  Signal,
  UnifiedSignalDefinition
> {
  ...
  for (const signal of SIGNALS_ALL) {
    if (SIGNAL_DEFINITIONS[signal]) {
      ...
      throw new Error(`Missing signal definition for ${signal}`);
    }
  }
