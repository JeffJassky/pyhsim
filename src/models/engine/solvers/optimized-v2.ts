...
/**
 * OPTIMIZED SOLVER V2 - UNIFIED ENGINE
 *
 * Single code path for both production and testing.
 * Uses vectorized RK4 + analytical Bateman PK throughout.
 */
...
export function createVectorLayout(signals: readonly Signal[], auxKeys: string[], receptorKeys: string[]): VectorLayout {
  const layout: VectorLayout = { size: 0, signals: new Map(), auxiliary: new Map(), receptors: new Map(), keys: [] };
  for (const s of signals) { layout.signals.set(s, layout.size); layout.keys[layout.size] = s; layout.size++; }
  for (const k of auxKeys) { layout.auxiliary.set(k, layout.size); layout.keys[layout.size] = k; layout.size++; }
  for (const k of receptorKeys) { layout.receptors.set(k, layout.size); layout.keys[layout.size] = k; layout.size++; }
  return layout;
}

export function stateToVector(state: SimulationState, layout: VectorLayout): Float64Array {
...
export function vectorToState(vector: Float64Array, layout: VectorLayout): SimulationState {
  const state: any = { signals: {}, auxiliary: {}, receptors: {}, pk: {}, accumulators: {} };
...
export interface PreResolvedDefinition {
  ...
  production: Array<{ coefficient: number; sourceIndex: number; transform?: (v: number, state: any, ctx: any) => number; }>;
  clearance: Array<{ rate: number; type: string; enzymeIndex: number; Km?: number; transform?: (v: number, state: any, ctx: any) => number; }>;
  couplings: Array<{ sourceIndex: number; receptorIndex: number; normalizedStrength: number; isStimulate: boolean; }>;
  min: number;
  max: number;
}
...
function resolveDefinitions(
  ...
  const resSignals: PreResolvedDefinition[] = SIGNALS_ALL.map(s => {
  ...
  const resAux: PreResolvedDefinition[] = Object.keys(auxDefinitions).map(k => {
...
function extractPKAgents(interventions: ActiveIntervention[], ctx: DynamicsContext): PKAgent[] {
  ...
  for (const iv of interventions) {
    ...
    const dose = Number(iv.params?.mg ?? iv.params?.dose ?? iv.params?.units ?? 100);
    ...
    if (vol) {
      ...
      switch (vol.kind) {
        case "tbw": vd = tbw * (vol.fraction ?? 0.6); break;
        case "lbm": vd = lbm * (vol.base_L_kg ?? 1.0); break;
        case "weight": vd = weight * (vol.base_L_kg ?? 0.7); break;
        case "sex-adjusted": vd = weight * ((ctx.subject?.sex ?? "male") === "male" ? vol.male_L_kg : vol.female_L_kg); break;
      }
    }
...
function computeAnalyticalConcentration(agents: PKAgent[], id: string, t: number): number {
  let sum = 0;
  ...
  for (const a of agents) {
    ...
    if (a.model === 'activity-dependent' || a.delivery === 'continuous') {
      ...
      if (dt <= a.duration) {
        ...
        sum += a.intensity * (1 - Math.exp(-a.duration / tau)) * Math.exp(-(dt - a.duration) / tau);
      }
      ...
      sum += Math.max(0, (dose / a.vd) * (a.ka / denom) * (Math.exp(-a.ke * dt) - Math.exp(-a.ka * dt)));
    } else {
      ...
      if (dt <= D) {
        sum += (kin / vd) * ((1 - Math.exp(-ke * dt)) / ke - (Math.exp(-ka * dt) - Math.exp(-ke * dt)) / denom);
      } else {
        const G_peak = (kin / ka) * (1 - Math.exp(-ka * D));
        const C_peak = (kin / vd) * ((1 - Math.exp(-ke * D)) / ke - (Math.exp(-ka * D) - Math.exp(-ke * D)) / denom);
        const dt_post = dt - D;
        sum += C_peak * Math.exp(-ke * dt_post) + (ka * G_peak / (vd * denom)) * (Math.exp(-ka * dt_post) - Math.exp(-ke * dt_post));
      }
    }
  }
...
function getStateProxy(vector: Float64Array, layout: VectorLayout, pkConcentrations: Map<string, number>): SimulationState {
  ...
  cachedPkConc = pkConcentrations;
  cachedStateProxy = {
    signals: new Proxy({}, { get: (_, p) => cachedVector![cachedLayout!.signals.get(p as Signal)!] ?? 0 }),
    auxiliary: new Proxy({}, { get: (_, p) => cachedVector![cachedLayout!.auxiliary.get(p as string)!] ?? 0 }),
    receptors: new Proxy({}, { get: (_, p) => cachedVector![cachedLayout!.receptors.get(p as string)!] ?? 0 }),
...
function getSignalTargets(target: string, layout: VectorLayout): Array<{ index: number; sign: number }> {
...
export function computeDerivativesVector(
  ...
  for (const iv of interventions) {
    if (iv.pharmacology?.pk) {
      pkConcentrations.set(`${iv.id}_central`, computeAnalyticalConcentration(pkAgents, iv.id, t));
    }
  }
  ...
  for (let i = 0; i < resolvedSignals.length; i++) {
    ...
    if (debug.enableBaselines !== false) {
      ...
      if (debug.enableConditions !== false && conditionAdjustments?.baselines?.[signalKey]?.amplitude) {
    ...
    if (debug.enableBaselines !== false) {
      for (const p of rd.production) {
        ...
        dS += p.coefficient * (p.transform ? p.transform(srcVal, stateProxy, ctx) : srcVal);
      }
    }
    ...
    if (debug.enableCouplings !== false) {
      for (const cp of rd.couplings) {
        ...
        const sensitivity = (debug.enableReceptors !== false && cp.receptorIndex !== -1) ? state[cp.receptorIndex] : 1.0;
        ...
        if (iv.pharmacology?.pd) {
          ...
          if (conc > 0) {
            for (const eff of iv.pharmacology.pd) {
              const targets = getSignalTargets(eff.target, layout);
              for (const tgt of targets) {
                if (tgt.index === rd.index) {
                  const dIdx = layout.receptors.get(`${eff.target}_density`);
                  const density = (debug.enableReceptors !== false && dIdx !== undefined) ? state[dIdx] : 1.0;

                  let response: number;
                  const isActivityDependent = iv.pharmacology?.pk?.model === 'activity-dependent' || iv.pharmacology?.pk?.delivery === 'continuous';
                  ...
                  if (isActivityDependent) {
                    ...
                    if (molarMass && molarMass > 0) {
                      if (eff.unit === "nM") effectiveConc = (conc / molarMass) * 1000000;
                      else if (eff.unit === "uM" || eff.unit === "µM") effectiveConc = (conc / molarMass) * 1000;
                    }
  
                    const EC50 = eff.EC50 ?? eff.Ki ?? 100;
                    const occupancy = effectiveConc / (effectiveConc + EC50);
                    const efficacy = eff.tau ?? 10;
                    response = (occupancy * efficacy / (occupancy * efficacy + occupancy + 1)) * (eff.intrinsicEfficacy ?? 50) * density / rd.tau;
                  }
  ...
  for (let i = 0; i < resolvedAux.length; i++) {
    ...
    if (debug.enableHomeostasis !== false) {
      ...
      for (const p of rd.production) {
        const srcVal = p.sourceIndex === -1 ? 1.0 : state[p.sourceIndex];
        dA += p.coefficient * (p.transform ? p.transform(srcVal, stateProxy, ctx) : srcVal);
      }
      for (const c of rd.clearance) {
        let rate = c.rate;
        if (c.type === 'saturable') rate /= (c.Km! + val);
          ...
          if (conc > 0) {
            ...
            for (const eff of iv.pharmacology.pd) {
              ...
              if (eff.target === auxKey) {
                ...
                const occupancy = effectiveConc / (effectiveConc + EC50);
                const efficacy = eff.tau ?? 10;
                const response = (occupancy * efficacy / (occupancy * efficacy + occupancy + 1)) * (eff.intrinsicEfficacy ?? 1.0) / rd.tau;
                if (eff.mechanism === "agonist" || eff.mechanism === "PAM") dA += response;
                else if (eff.mechanism === "antagonist" || eff.mechanism === "NAM") dA -= response * (val / (val + 0.1));
              }
            }
          }
        }
...
export function integrateStepVector(
  state: Float64Array, t: number, dt: number, ctx: DynamicsContext, interventions: ActiveIntervention[],
  layout: VectorLayout, resSignals: PreResolvedDefinition[], resAux: PreResolvedDefinition[],
  options?: any, conditionAdjustments?: any
) {
  if (dt === 0) return;
  ...
  // RK4 integration
  computeDerivativesVector(state, t, ctx, interventions, k1_ws, layout, resSignals, resAux, opts, conditionAdjustments);
  for (let i = 0; i < layout.size; i++) tmp_ws[i] = state[i] + k1_ws[i] * (dt / 2);
  computeDerivativesVector(tmp_ws, t + dt / 2, ctx, interventions, k2_ws, layout, resSignals, resAux, opts, conditionAdjustments);
  for (let i = 0; i < layout.size; i++) tmp_ws[i] = state[i] + k2_ws[i] * (dt / 2);
  computeDerivativesVector(tmp_ws, t + dt / 2, ctx, interventions, k3_ws, layout, resSignals, resAux, opts, conditionAdjustments);
  for (let i = 0; i < layout.size; i++) tmp_ws[i] = state[i] + k3_ws[i] * dt;
  computeDerivativesVector(tmp_ws, t + dt, ctx, interventions, k4_ws, layout, resSignals, resAux, opts, conditionAdjustments);
  for (let i = 0; i < layout.size; i++) state[i] += (dt / 6) * (k1_ws[i] + 2 * k2_ws[i] + 2 * k3_ws[i] + k4_ws[i]);

  // Clamp values
  resSignals.forEach(rs => state[rs.index] = Math.max(rs.min, Math.min(rs.max, state[rs.index])));
  resAux.forEach(ra => state[ra.index] = Math.max(0, Math.min(2.0, state[ra.index])));
}
...
export function integrateStep(
  ...
  const layout = createVectorLayout(SIGNALS_ALL, Object.keys(auxDefinitions), Object.keys(state.receptors || {}));
  const { resSignals, resAux } = resolveDefinitions(definitions, auxDefinitions, layout);
  ...
  for (let s = 0; s < subSteps; s++) {
    ...
    integrateStepVector(vector, subT, subDt, ctx, interventions, layout, resSignals, resAux, { debug });
  }
  ...
  for (const iv of interventions) {
    if (iv.pharmacology?.pk) {
      result.pk[`${iv.id}_central`] = computeAnalyticalConcentration(pkAgents, iv.id, t + dt);
...
export function computeDerivatives(
  ...
  computeDerivativesVector(vector, t, ctx, interventions, derivs, layout, resSignals, resAux, { debug, pkAgents });
  ...
  for (const iv of interventions) {
    if (iv.pharmacology?.pk) {
      ...
      if (pk.model === 'activity-dependent' || pk.delivery === 'continuous') {
        ...
        const nextConc = computeAnalyticalConcentration(pkAgents, iv.id, t + 0.001);
...
export function runOptimizedV2(request: WorkerComputeRequest): WorkerComputeResponse {
  ...
  if (enableInterventions) {
    ...
    for (let d = 0; d < numDays; d++) {
      for (const item of items) {
        ...
        const pharms: any[] = (item as any).resolvedPharmacology ||
          (def.pharmacology && typeof def.pharmacology !== 'function' ? [def.pharmacology] : []);
        ...
        for (let idx = 0; idx < pharms.length; idx++) {
          ...
          const dose = Number(item.meta.params?.mg ?? item.meta.params?.dose ?? item.meta.params?.units ?? 100);
          const F = pk.bioavailability ?? 1.0;
          const ke = pk.eliminationRate ?? 0.693 / (pk.halfLifeMin ?? 60);
          const ka = pk.absorptionRate ?? (ke * 4);
  ...
  const layout = createVectorLayout(SIGNALS_ALL, Object.keys(initialObjState.auxiliary), Object.keys(initialObjState.receptors));
  const currentStateVector = stateToVector(initialObjState, layout);
  const { resSignals, resAux } = resolveDefinitions(unifiedDefinitions, AUXILIARY_DEFINITIONS, layout);
  ...
  const updateActiveSet = (t: number) => {
    ...
    while (activePointer < allActiveIvs.length && allActiveIvs[activePointer].startTime <= t) {
      if (t <= allActiveIvs[activePointer].startTime + allActiveIvs[activePointer].duration) {
  ...
  const conditionAdjustments = (options?.debug?.enableConditions !== false) ? {
  ...
  if (options?.initialHomeostasisState) {
    ...
    if (s.signals) Object.entries(s.signals).forEach(([k, v]) => {
      const idx = layout.signals.get(k as any);
      if (idx !== undefined) currentStateVector[idx] = v as number;
    });
    if (s.auxiliary) Object.entries(s.auxiliary).forEach(([k, v]) => {
    ...
    for (let t_warm = -1440; t_warm < 0; t_warm += 1.0) {
      ...
      const isAsleep_warm = enableInterventions && items.some(item => {
        ...
        return (wallMin >= item.startMin && wallMin <= item.startMin + item.durationMin);
      });
      ...
      for (const iv of activeSet) {
        if (iv.pharmacology?.pk) {
          pkConcentrations.set(`${iv.id}_central`, getAnalyticalConc(iv.id, t_warm));
        }
      }

      computeDerivativesVector(currentStateVector, t_warm, warmUpCtx, activeSet, k1_ws, layout, resSignals, resAux,
        { debug: options?.debug, pkAgents }, conditionAdjustments);
      for (let i = 0; i < layout.size; i++) currentStateVector[i] += k1_ws[i];
      resSignals.forEach(rs => currentStateVector[rs.index] = Math.max(rs.min, Math.min(rs.max, currentStateVector[rs.index])));
      resAux.forEach(ra => currentStateVector[ra.index] = Math.max(0, Math.min(2.0, currentStateVector[ra.index])));
    }
  }

  // Output arrays
  const series: Record<Signal, Float32Array> = {} as any;
  includeSignals.forEach(s => series[s] = new Float32Array(gridMins.length));
  const auxSeries: Record<string, Float32Array> = {};
  Object.keys(AUXILIARY_DEFINITIONS).forEach(k => auxSeries[k] = new Float32Array(gridMins.length));
  const homeoSeries = {
  ...
  gridMins.forEach((minute, idx) => {
    ...
    if (dt > 0) {
      ...
      for (let s = 0; s < subSteps; s++) {
        ...
        computeDerivativesVector(currentStateVector, t, dCtx, activeSet, k1_ws, layout, resSignals, resAux, opts, conditionAdjustments);
        for (let i = 0; i < layout.size; i++) tmp_ws[i] = currentStateVector[i] + k1_ws[i] * (subDt / 2);
        computeDerivativesVector(tmp_ws, t + subDt / 2, dCtx, activeSet, k2_ws, layout, resSignals, resAux, opts, conditionAdjustments);
        for (let i = 0; i < layout.size; i++) tmp_ws[i] = currentStateVector[i] + k2_ws[i] * (subDt / 2);
        computeDerivativesVector(tmp_ws, t + subDt / 2, dCtx, activeSet, k3_ws, layout, resSignals, resAux, opts, conditionAdjustments);
        for (let i = 0; i < layout.size; i++) tmp_ws[i] = currentStateVector[i] + k3_ws[i] * subDt;
        computeDerivativesVector(tmp_ws, t + subDt, dCtx, activeSet, k4_ws, layout, resSignals, resAux, opts, conditionAdjustments);
        for (let i = 0; i < layout.size; i++) currentStateVector[i] += (subDt / 6) * (k1_ws[i] + 2 * k2_ws[i] + 2 * k3_ws[i] + k4_ws[i]);
    ...
    layout.auxiliary.forEach((idxV, k) => { if (auxSeries[k]) auxSeries[k][idx] = currentStateVector[idxV]; });

    const getV = (key: string, map: Map<string, number>) => {
      const idxV = map.get(key);
      return idxV !== undefined ? currentStateVector[idxV] : 0;
    };
    homeoSeries.glucosePool[idx] = getV('glucose', layout.signals);
    homeoSeries.insulinPool[idx] = getV('insulin', layout.signals);
    homeoSeries.hepaticGlycogen[idx] = getV('hepaticGlycogen', layout.auxiliary);
    homeoSeries.adenosinePressure[idx] = getV('adenosinePressure', layout.auxiliary);
    homeoSeries.cortisolPool[idx] = getV('cortisol', layout.signals);
    homeoSeries.cortisolIntegral[idx] = getV('cortisolIntegral', layout.auxiliary);
    homeoSeries.adrenalineReserve[idx] = getV('adrenaline', layout.signals);
    homeoSeries.dopamineVesicles[idx] = getV('dopamineVesicles', layout.auxiliary);
    homeoSeries.norepinephrineVesicles[idx] = getV('norepinephrineVesicles', layout.auxiliary);
    homeoSeries.serotoninPrecursor[idx] = getV('serotoninPrecursor', layout.auxiliary);
    homeoSeries.acetylcholineTone[idx] = getV('acetylcholine', layout.signals);
    homeoSeries.gabaPool[idx] = getV('gabaPool', layout.auxiliary);
    homeoSeries.bdnfExpression[idx] = getV('bdnfExpression', layout.auxiliary);
    homeoSeries.ghReserve[idx] = getV('ghReserve', layout.auxiliary);
  });
  ...
  return {
    ...
    homeostasisSeries: homeoSeries as any
  };
}
