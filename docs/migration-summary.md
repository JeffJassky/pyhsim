...
*   **Interventions Model**: `src/models/interventions.ts` was refactored to:
    ...
    *   This means kernel functions are now *generated* from metadata like `halfLifeMin` and `bioavailability`, rather than being hardcoded strings.
...
## Next Steps (User)
*   **Tune Effect Gains**: The `effectGain` parameter in `PharmacologyDef` controls the magnitude of the signal response. These are currently estimated (e.g. 50.0 for generic, 2.5 for Alcohol). Adjust these to match desired visual output.
