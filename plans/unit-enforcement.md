...
However, **Pharmacokinetics (PK)** of interventions *must* scale with body size.
...
3.  *User Experience:* Users immediately see "10 nM" instead of "50%", even though the engine is still running on "50".
...
3.  **Refactor Interventions:** Scale `effectGain` in `interventions/*.ts`.
