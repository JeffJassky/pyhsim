...
import type { Signal, PharmacologyDef } from '@/types';
...
describe('Intervention Unit Integrity', () => {
  it('should have matching units for all pharmacological effects', () => {
    ...
    INTERVENTIONS.forEach((intervention) => {
      ...
      if (typeof intervention.pharmacology === 'function') {
        const defaultParams = Object.fromEntries(
           intervention.params.map(p => [p.key, p.default ?? 0])
        );
        const result = (intervention.pharmacology as any)(defaultParams);
        pharms = Array.isArray(result) ? result : [result];
      } else {
        pharms = [intervention.pharmacology];
      }
