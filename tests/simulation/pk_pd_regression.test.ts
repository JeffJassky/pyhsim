...
describe('PK/PD Regression Tests', () => {
  ...
  const createCtx = (minuteOfDay: number, isAsleep = false): DynamicsContext => ({
    minuteOfDay,
  ...
  function simulate(
    ...
    for (let i = 0; i < durationMin; i++) {
      ...
      state = integrateStep(state, t, dt, ctx, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, interventions);
    }
    return state;
  }
      ...
      const interventions: ActiveIntervention[] = [
        {
          ...
          pharmacology: {
            ...
            pd: [{ target: 'DAT', mechanism: 'antagonist', Ki: 0.01, effectGain: 30.0 }],
          },
        },
        {
  ...
  describe('Caffeine Adenosine Antagonism Chain', () => {
    ...
    it('should affect dopamine via adenosine receptor antagonism', () => {
      ...
      expect(afterState.pk['caffeine-dopamine-test_central']).toBeGreaterThan(0.1);
    });
  });
  ...
  describe('Activity-Dependent Interventions', () => {
    it('should handle sleep intervention with activity-dependent PK', () => {
      ...
      const sleepIntervention: ActiveIntervention[] = [{
        ...
        pharmacology: {
          ...
          pd: [
            { target: 'melatonin', mechanism: 'agonist', effectGain: 50.0, tau: 10 },
            { target: 'gaba', mechanism: 'agonist', effectGain: 30.0, tau: 10 },
          ],
        },
      }];
      ...
      for (let i = 0; i < 60; i++) {
        sleepState = integrateStep(sleepState, i, 1.0, ctx, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, sleepIntervention);
      }
      ...
      expect(sleepState.signals.melatonin).toBeGreaterThan(state.signals.melatonin);
    });
    ...
    it('should handle multi-day sleep interventions without overwriting', () => {
      ...
      const sleepInterventions: ActiveIntervention[] = [
        {
          ...
          pharmacology: {
            pk: { model: 'activity-dependent' },
            pd: [{ target: 'melatonin', mechanism: 'agonist', effectGain: 50.0, tau: 10 }],
          },
        },
        {
      ...
      let sleepState = state;
      for (let i = 0; i < 60; i++) {
        sleepState = integrateStep(sleepState, 1320 + i, 1.0, ctx, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, sleepInterventions);
      }
