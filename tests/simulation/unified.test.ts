...
describe('Unified ODE Architecture', () => {
  ...
  function simulate(initialState: SimulationState, durationMin: number, dt: number = 1.0, currentCtx: DynamicsContext = ctx) {
    let state = initialState;
    const steps = Math.ceil(durationMin / dt);
    for (let i = 0; i < steps; i++) {
      const t = i * dt;
      state = integrateStep(state, t, dt, { 
        ...currentCtx, 
  ...
  it('should initialize state correctly', () => {
    const state = createInitialState(SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, {
      subject,
      physiology,
      isAsleep: false
    });
  ...
  it('should accumulate adenosine pressure while awake', () => {
    ...
    expect(nextState.auxiliary.adenosinePressure).toBeGreaterThan(state.auxiliary.adenosinePressure);
  });
  ...
  it('should respond to interventions as forcing functions', () => {
    ...
    // Step for 20 minutes
    for (let i = 0; i < 20; i++) {
      nextState = integrateStep(nextState, 600 + i, 1.0, { ...ctx, minuteOfDay: 600 + i, circadianMinuteOfDay: 600 + i }, definitions, AUXILIARY_DEFINITIONS, interventions);
    }
  ...
  it('should respond to mechanistic pharmacology (PK/PD)', () => {
    ...
    // Step for 10 minutes
    for (let i = 0; i < 10; i++) {
      nextState = integrateStep(nextState, 600 + i, 1.0, { ...ctx, minuteOfDay: 600 + i, circadianMinuteOfDay: 600 + i }, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, interventions);
    }
  ...
  it('should respond to activity-dependent interventions (sleep)', () => {
    ...
    const interventions: any[] = [
      {
        ...
        pharmacology: {
          ...
          pd: [
            ...
            { target: 'histamine', mechanism: 'antagonist', effectGain: 30.0 },
            { target: 'orexin', mechanism: 'antagonist', effectGain: 35.0 },
          ]
        }
      }
    ];
    ...
    // Step for 30 minutes
    for (let i = 0; i < 30; i++) {
      nextState = integrateStep(nextState, i, 1.0, { ...ctx, minuteOfDay: i, circadianMinuteOfDay: i }, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, interventions);
    }
  ...
  it('should respond to caffeine intervention with proper dosing', () => {
    ...
    // Step for 60 minutes
    for (let i = 0; i < 60; i++) {
      nextState = integrateStep(nextState, 420 + i, 1.0, { ...ctx, minuteOfDay: 420 + i, circadianMinuteOfDay: 420 + i }, SIGNAL_DEFINITIONS, AUXILIARY_DEFINITIONS, interventions);
    }
